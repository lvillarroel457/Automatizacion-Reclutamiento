...
class OpenAiRole(Role):
    ...
    def parse_role(self, raw_role):
        ...
        user_prompt = f"A partir de este texo: { raw_role }, genera un diccionario de pyhton en formato Json object que repsesente los requisitos para el puesto de trabajo. Que cada llave sea el nombre de un requisito ( una palabra que capture lo escencial del requisito) y el valor la descripcion del requisito que indique los requisistos correspondientes que se requieren. Que la primera llave sea 'Cargo' y tenga como valor el nombre del cargo. MUY IMPORTANTE: Retorna solo el diccionario (Json object) en formato string , sin nada de texto extra, por ejemplo: {{'Cargo': 'Nombre del cargo', 'Estudios': 'Ser titular de una licenciatura en Ciencias de la Computación o un campo relacionado',...}} "
